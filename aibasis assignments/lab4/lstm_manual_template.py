...
class LSTM(nn.Module):
    ...
    def __init__(self, input_size, hidden_size):
        ...
        self.flinear = nn.Linear(input_size + hidden_size, hidden_size)
        self.ilinear1 = nn.Linear(input_size + hidden_size, hidden_size)
        self.ilinear2 = nn.Linear(input_size + hidden_size, hidden_size)
        self.olinear = nn.Linear(input_size + hidden_size, hidden_size)
    ...
    def forward(self, x):
        ...
        for j in range(seq_lenth):
            information = torch.cat((h, x[:, j, :]), dim=1)
            f = self.sigmoid(self.flinear(information))
            i = self.sigmoid(self.ilinear1(information))
            c_cur = self.tanh(self.ilinear2(information))
            o = self.sigmoid(self.olinear(information))
            c = f * c + i * c_cur
            h = o * self.tanh(c) 
        return h
...
class Net(nn.Module):
    ...
    def __init__(self, embedding_size=128, hidden_size=64, num_classes=2):
...
optimizer = torch.optim.Adam(net.parameters(), lr=1e-3 * 5, weight_decay=0.0)
gamma = 0.7
